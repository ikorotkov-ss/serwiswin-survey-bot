...
- Установит Python, ffmpeg и системные зависимости
- Скачает модель распознавания речи
- Скопирует код бота
- Настроит автозапуск (systemd)
- Настроит ежедневный бэкап базы данных

По умолчанию проект рассчитан на лёгкий режим для недорогого VPS: бот сохраняет голосовые файлы и не расшифровывает их во время опроса. Whisper нужен только для полного режима или для отдельной постобработки аудио после завершения опроса.
...
Для полного режима с онлайн-расшифровкой голосовых нужен более мощный сервер. На слабом VPS лучше не запускать whisper во время опроса: при нескольких одновременных голосовых бот может начать отвечать с задержками.
